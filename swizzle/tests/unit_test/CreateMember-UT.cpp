...
namespace {
    ...
    struct WhenNodeStackIsEmpty : public WhenTokenStackIsEmpty
    {
    ...
    TEST_FIXTURE(WhenNodeStackIsEmpty, verifyCreateMemberThrows)
    {
        CHECK_THROW(detail::createMember(token, nodeStack, tokenStack, context), swizzle::SyntaxError);
    }

    struct WhenTopOfNodeStackIsNotStructField
    {
    ...
    struct WhenSecondItemInNodeStackIsNotStruct
    {
